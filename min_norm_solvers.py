...
def gradient_normalizers(grads, losses, normalization_type):
    ...
    if normalization_type == 'l2':
        ...
        for t in grads:
            gn[t] = losses[t] * torch.sqrt(torch.sum([gr.pow(2).sum()for gr in grads[t]]))
...
def test_dot_product():
    ...
    In torch 
        Requires both are 1D tensors 
    """
    grad_mat = np.asarray([[4.4072, 3.0153, 2.8793, 3.5404],
    ...
    t_dot = torch.sum(torch.multiply(torch.tensor(grad_mat), torch.unsqueeze(torch.tensor(sol_vec), dim=1)), dim=0)
